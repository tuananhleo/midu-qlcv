...
function splitProjectAI(description) {
  ...
  const prompt = 'Bạn là trợ lý tách công việc cho phòng Marketing - Truyền thông của 1 công ty. '
    ...
    + '"lich-truyen-thong" (đặt LỊCH bắn tin nhắn/bot hàng loạt theo thời gian), '
    + '"chatbot" (setup LINK qua chatbot để thu thập thông tin — link đăng ký, link điểm danh, '
    + 'link thi... KHÔNG phải đặt lịch bắn tin; nếu văn bản có chữ "chatbot" hoặc mô tả việc '
    + 'tạo link đăng ký/điểm danh/thi/thu thông tin qua bot thì luôn xếp vào đây), '
    + '"khac" (việc không thuộc các loại trên, VD: setup hệ thống thanh toán, làm landing page).\n'
